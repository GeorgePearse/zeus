  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        const { InviteEmail } = await import("@zeus-ai/console-mail/InviteEmail.jsx")
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on Zeus`,
          body: render(
            // @ts-ignore
            InviteEmail({
              inviter: emailInfo.email,
              assetsUrl: `https://zeus.ai/email`,
