...
import { createOpencode, type ToolPart } from "@zeus-ai/sdk"
...
console.log("🚀 Starting zeus server...")
...
;(async () => {
  const events = await zeus.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new zeus session...")
    const { client, server } = zeus
    ...
    console.log("✅ Created zeus session:", createResult.data.id)
  ...
  console.log("📝 Sending to zeus:", message.text)
