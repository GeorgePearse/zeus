...
type App struct {
	...
	Model             *zeus.Model
	Session           *zeus.Session
	Messages          []Message
	Permissions       []zeus.Permission
	CurrentPermission zeus.Permission
...
type SessionSelectedMsg = *zeus.Session
...
type PermissionRespondedToMsg struct {
	Response zeus.SessionPermissionRespondParamsResponse
}
...
func New(
	...
	configInfo, err := httpClient.Config.Get(ctx, zeus.ConfigGetParams{})
	if err != nil {
		return nil, err
	}
	...
	themeEnv := os.Getenv("ZEUS_THEME")
	if themeEnv != "" {
		appState.Theme = themeEnv
	}

	agentIndex := slices.IndexFunc(agents, func(a zeus.Agent) bool {
		return a.Mode != "subagent"
	})
	var agent *zeus.Agent
	modeName := "build"
	if appState.Agent != "" {
	...
	customCommands, err := httpClient.Command.List(ctx, zeus.CommandListParams{})
	if err != nil {
		return nil, err
	}
...
func findProviderByID(providers []zeus.Provider, providerID string) *zeus.Provider {
	for _, provider := range providers {
		if provider.ID == providerID {
			return &provider
		}
...
func (a *App) InitializeProvider() tea.Cmd {
	providersResponse, err := a.Client.App.Providers(context.Background(), zeus.AppProvidersParams{})
	if err != nil {
	...
	var selectedProvider *zeus.Provider
	var selectedModel *zeus.Model
...
func getDefaultModel(
	response *zeus.AppProvidersResponse,
	provider zeus.Provider,
) *zeus.Model {
...
func (a *App) IsBusy() bool {
	...
	if casted, ok := lastMessage.Info.(zeus.AssistantMessage); ok {
...
func (a *App) HasAnimatingWork() bool {
	for _, msg := range a.Messages {
		...
		for _, p := range msg.Parts {
			if tp, ok := p.(zeus.ToolPart); ok {
				if tp.State.Status == zeus.ToolPartStateStatusPending {
					return true
				}
			}
		}
...
func (a *App) InitializeProject(ctx context.Context) tea.Cmd {
	...
	go func() {
		_, err := a.Client.Session.Init(ctx, a.Session.ID, zeus.SessionInitParams{
			MessageID:  zeus.F(id.Ascending(id.Message)),
			ProviderID: zeus.F(a.Provider.ID),
			ModelID:    zeus.F(a.Model.ID),
		})
		if err != nil {
...
func (a *App) CreateSession(ctx context.Context) (*zeus.Session, error) {
	session, err := a.Client.Session.New(ctx, zeus.SessionNewParams{})
	if err != nil {
		return nil, err
	}
	return session, nil
...
func (a *App) SendPrompt(ctx context.Context, prompt Prompt) (*App, tea.Cmd) {
	...
	cmds = append(cmds, func() tea.Msg {
		_, err := a.Client.Session.Prompt(ctx, a.Session.ID, zeus.SessionPromptParams{
			Model: zeus.F(zeus.SessionPromptParamsModel{
				ProviderID: zeus.F(a.Provider.ID),
				ModelID:    zeus.F(a.Model.ID),
			}),
			Agent:     zeus.F(a.Agent().Name),
			MessageID: zeus.F(messageID),
			Parts:     zeus.F(message.ToSessionChatParams()),
		})
		if err != nil {
...
func (a *App) SendCommand(ctx context.Context, command string, args string) (*App, tea.Cmd) {
	...
	cmds = append(cmds, func() tea.Msg {
		params := zeus.SessionCommandParams{
			Command:   zeus.F(command),
			Arguments: zeus.F(args),
			Agent:     zeus.F(a.Agents[a.AgentIndex].Name),
		}
		if a.Provider != nil && a.Model != nil {
			params.Model = zeus.F(a.Provider.ID + "/" + a.Model.ID)
		}
...
func (a *App) Cancel(ctx context.Context, sessionID string) error {
	...
	_, err := a.Client.Session.Abort(ctx, sessionID, zeus.SessionAbortParams{})
	if err != nil {
...
func (a *App) ListSessions(ctx context.Context) ([]zeus.Session, error) {
	response, err := a.Client.Session.List(ctx, zeus.SessionListParams{})
	if err != nil {
		return nil, err
	}
	if response == nil {
		return []zeus.Session{}, nil
	}
	sessions := *response
	return sessions, nil
}
...
func (a *App) DeleteSession(ctx context.Context, sessionID string) error {
	_, err := a.Client.Session.Delete(ctx, sessionID, zeus.SessionDeleteParams{})
	if err != nil {
...
func (a *App) UpdateSession(ctx context.Context, sessionID string, title string) error {
	_, err := a.Client.Session.Update(ctx, sessionID, zeus.SessionUpdateParams{
		Title: zeus.F(title),
	})
	if err != nil {
...
func (a *App) ListMessages(ctx context.Context, sessionId string) ([]Message, error) {
	response, err := a.Client.Session.Messages(ctx, sessionId, zeus.SessionMessagesParams{})
	if err != nil {
		return nil, err
	}
	if response == nil {
...
func (a *App) ListProviders(ctx context.Context) ([]zeus.Provider, error) {
	response, err := a.Client.App.Providers(ctx, zeus.AppProvidersParams{})
	if err != nil {
		return nil, err
	}
	if response == nil {
		return []zeus.Provider{}, nil
	}
