...
import { createOpencodeClient } from "@zeus-ai/sdk"
...
try {
  ...
  console.log("zeus session", session.id)
...
function createOpencode() {
  ...
  const proc = spawn(`zeus`, [`serve`, `--hostname=${host}`, `--port=${port}`])
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/zeus|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/zeus` or `/oc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to zeus server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.zeus.ai" : "https://zeus.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.zeus.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("zeus-github-action")
    response = await fetch("https://api.zeus.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  let prompt = (() => {
    const payload = useContext().payload as IssueCommentEvent
    const body = payload.comment.body.trim()
    if (body === "/zeus" || body === "/oc") return "Summarize this thread"
    if (body.includes("/zeus") || body.includes("/oc")) return body
    throw new Error("Comments must mention `/zeus` or `/oc`")
  })()
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to zeus...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "zeus-agent[bot]"`
  await $`git config --global user.email "zeus-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `zeus/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/zeus-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[zeus session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
